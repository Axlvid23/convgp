...
class CasermExperiment(exp_tools.CasermExperiment):
    def __init__(self, name=None, M=100, run_settings=None):
        ...
        super(CasermExperiment, self).__init__(name)
    ...
    def setup_model(self):
        ...
        if self.run_settings['kernel'] == "rbf":
            k = GPflow.kernels.RBF(214)
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings['kernel'] == "poly":
            k = GPflow.kernels.Polynomial(214, degree=9.0)
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings['kernel'] == "rbfpoly":
            k = (GPflow.kernels.Polynomial(214, degree=9.0) + GPflow.kernels.RBF(28 * 28) +
                 GPflow.kernels.White(214, 1e-1))
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings["kernel"] == "conv":
            k = ckern.ConvRBF([214,1],[1,1]) + GPflow.kernels.White(1, 1e-3)
        ...
        self.m = GPflow.svgp.SVGP(self.X, self.Y, k, GPflow.likelihoods.MultiClass(3), Z.copy(), num_latent=3,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run casern experiment.')
    ...
    exp = CasermExperiment(name=args.name, M=args.M, run_settings=run_settings)
