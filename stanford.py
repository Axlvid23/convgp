...
class StanfordExperiment(exp_tools.StanfordExperiment):
    def __init__(self, name=None, M=100, run_settings=None):
        name = "stanford-%s%i" % (run_settings['kernel'], M) if name is None else name
        super(StanfordExperiment, self).__init__(name)
    ...
    def setup_model(self):
        ...
        if self.run_settings['kernel'] == "rbf":
            k = GPflow.kernels.RBF(7 * 7)
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings['kernel'] == "poly":
            k = GPflow.kernels.Polynomial(7 * 7, degree=9.0)
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings['kernel'] == "rbfpoly":
            k = (GPflow.kernels.Polynomial(7 * 7, degree=9.0) + GPflow.kernels.RBF(7 * 7) +
                 GPflow.kernels.White(7 * 7, 1e-1))
            Z = self.X[np.random.permutation(len(self.X))[:self.M], :]
        elif self.run_settings["kernel"] == "conv":
            k = ckern.ConvRBF([7, 7], [5, 5]) + GPflow.kernels.White(1, 1e-3)
        elif self.run_settings['kernel'] == "wconv":
            k = ckern.WeightedConv(GPflow.kernels.RBF(25), [7, 7], [5, 5]) + GPflow.kernels.White(1, 1e-3)
        else:
        ...
        self.m = GPflow.svgp.SVGP(self.X, (self.Y), k, GPflow.likelihoods.MultiClass(5), Z.copy(), num_latent=5,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run stanford experiment.')
    ...
    exp = StanfordExperiment(name=args.name, M=args.M, run_settings=run_settings)
